...
export default function FlashcardAberto(props) {
  const { flashcard, querMostraAResposta, setQuerMostraAResposta } = props;
  ...
  return (
    ...
    >
      <p
        data-identifier={
          querMostraAResposta ? "flashcard-answer" : "flashcard-question"
        }
      >
...
const PerguntaAberta = styled.li`
  ...
  transform: ${({ querMostraAResposta }) =>
    querMostraAResposta ? "rotateY(180deg)" : ""};
  ...
  img {
    display: ${({ querMostraAResposta }) =>
      querMostraAResposta ? "none" : "inline"};
    position: absolute;
    bottom: 10px;
    right: 10px;
  }
